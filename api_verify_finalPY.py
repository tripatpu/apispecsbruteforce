    ...
    def analyze(self, entries):
        ...
        for e in entries:
            ...
            if matches_ca:
                ...
                # KEY v5.1 FIX: Categorized paths MUST have matching content-type
                # swagger.json returning text/html is NEVER real, even with unique CL
                # Only paths that SHOULD return structured data need this check
                _ct_required_cats = {"swagger","openapi","graphql","actuator","metrics",
                                     "wsdl","proto","auth","config_leak","api_json","cloud_meta"}
                if not is_fp and e.path_category in _ct_required_cats:
                    if is_hct and e.content_score < 40:
                        # HTML response for a path that should return json/yaml/xml = FP
                        is_fp = True
                        reasons.append("FP:path_%s_returns_html_not_%s" % (
                            e.path_category, "/".join(_EXPECTED_CT.get(e.path_category,["structured"]))))
                    elif not is_sct and not is_hct and e.content_score < 40:
                        # Unknown/missing CT with no body proof = suspicious
                        # Only mark FP if body doesn't prove it's real
                        if e.content_score < 20:
                            is_fp = True
                            reasons.append("FP:path_%s_no_structured_ct_no_body_proof" % e.path_category)
...
def main():
    ...
    if args.ffuf_file:
        ...
        # Reprobe if requested
        if args.reprobe:
            if not HAS_REQUESTS:
                log("[!] requests library required for --reprobe. Install: pip install requests")
                sys.exit(1)
            hdrs = {}
            if args.headers:
                for h in args.headers:
            ...
            # Body analysis from ffuf data (no reprobe)
            for e in entries:
                if e.body:
                    a = BodyAnalyzer.analyze(e.body, e.content_type, e.fuzz_word)
                    e.content_analysis = a["detail"]; e.content_score = a["score"]
                    e.content_tags = "; ".join(a["tags"]) if a["tags"] else ""
        ...
        # Save output
        if args.output:
            write_results(entries, args.output, include_fp=args.include_fp)

    elif args.url and args.wordlist:
        # Live mode
        if not HAS_REQUESTS:
            log("[!] requests library required for live mode. Install: pip install requests")
            sys.exit(1)
        scanner = LiveScanner(args)
        scanner.run()
        if scanner.hits:
            print_hit_table(scanner.hits)
    else:
