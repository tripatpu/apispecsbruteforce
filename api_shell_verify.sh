...
OPTIONS:
  -f, --ffuf-file FILE    ffuf JSON output file (required)
...
# ========================= VALIDATION ======================================
if [[ -z "$FFUF_FILE" ]]; then
    err "Missing required option: -f/--ffuf-file"
    usage
fi
...
run_fp_engine() {
    ...
    while IFS= read -r line; do
        ...
        log "    CL=%-6s W=%-5s L=%-5s -> ${count}x (${pct}%%)" "$cl" "$words" "$lines"

        # Check if dominant (>= 10% or >= 10 count)
        if (( count >= 10 )) || (( $(echo "$pct >= 10.0" | bc -l) )); then
            dominant_fps+=("$fp_str")
    ...
    local catchall_pct
    catchall_pct=$(echo "scale=1; $catchall_total * 100 / $total" | bc)
    log "    Catch-all coverage: ${catchall_pct}% of all results"

    if (( $(echo "$catchall_pct >= 60.0" | bc -l) )); then
    ...
    echo "$entries_json" | jq -c '.[]' | while IFS= read -r entry; do
        ...
        if [[ "$matches_catchall" == "true" ]]; then
            ...
            local cl_pct
            cl_pct=$(echo "scale=1; $cl_freq * 100 / $total" | bc)

            if (( $(echo "$cl_pct >= 15.0" | bc -l) )) && is_html_ct "$ct" && [[ $content_score -lt 30 ]]; then
                is_fp=true
                reasons="${reasons}; FP:common_cl(${cl_pct}%)+html"
            elif (( $(echo "$cl_pct >= 30.0" | bc -l) )) && [[ $content_score -lt 30 ]]; then
                is_fp=true
        ...
        if [[ "$is_fp" == "true" ]] && [[ "$status" =~ ^(401|403|405)$ ]]; then
            ...
            local status_pct
            status_pct=$(echo "scale=2; $same_status_count * 100 / $total" | bc)
            if (( $(echo "$status_pct < 20.0" | bc -l) )); then
                is_fp=false
...
main() {
    ...
    # Parse ffuf JSON into normalized entries
    local entries_json
    ...
    if [[ "$REPROBE" == "true" ]]; then
        entries_json=$(reprobe_all "$entries_json")
    fi
    ...
    log ""
    log "  [Result] Total: $total | FPs: $fp_count ($(echo "scale=1; $fp_count * 100 / $total" | bc)%) | Verified: $verified_count"
